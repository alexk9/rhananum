		...
		if (cur>1&& strncmp(str,cur-2,"거",0,2)==0 && 	pcheck(str,cur,"rule_것")!=0) then
			if (str[cur]=='ᆸ') then
				if (strncmp(str,cur,"ᆸ니",0,3)==0) then
					...
					@mc.phoneme_change(from,buf,buf2,TagSet.TAG_TYPE_NBNP,TagSet.TAG_TYPE_JP,0);
				end
			else
				if (strncmp(str,cur,"ᆯ로",0,3)==0) then
					...
					buf = new_str[0,cur+1]
					buf2 = new_str[cur+1..-1]
					@mc.phoneme_change(from,buf,buf2,TagSet.TAG_TYPE_NBNP,TagSet.TAG_TYPE_JOSA,0);

				elsif (str[cur]=='ᆯ'||str[cur]=='ᆫ') then
  ...
  def rule_i(from, prev, str,cur)
		...
		if ((prev!=nil&&prev.length() != 0&&cur==0)	&&pcheck(prev,prev.length()-1,"중성")!=0) then
			if (strncmp(str,0,"여",0,2)==0) then
				...
				@mc.phoneme_change(from,buf,buf2,TagSet.TAG_TYPE_JP,TagSet.TAG_TYPE_EOMIES,0);
			else
				...
				@mc.phoneme_change(from,"이",str,TagSet.TAG_TYPE_JP,TagSet.TAG_TYPE_EOMIES,0);
				buf = "이" + str;
	...
	def rule_irr_word(from,prev,str,cur)
		...
		if ((cur>0&&cur<=len&&pcheck(str,cur-1,"l21")!=0)	&&pcheck(str,cur,"21")!=0	&&pcheck(str,cur+1,"r21")!=0) then
			...
			buf = new_str[0,cur]
			buf2 = new_str[cur..-1]
			@mc.phoneme_change(from,buf,buf2,TagSet.TAG_TYPE_YONGS,TagSet.TAG_TYPE_EOMIES,tagSet.IRR_TYPE_D);
		end
		...
		if ((cur>0&&cur<len&&pcheck(str,cur-1,"l22")!=0) &&pcheck(str,cur,"22")!=0 &&pcheck(str,cur+1,"r22")!=0) then
			...
			buf = new_str[0,cur+1]
			buf2 = new_str[cur+1..-1]
			@mc.phoneme_change(from,buf,buf2,TagSet.TAG_TYPE_YONGS,TagSet.TAG_TYPE_EOMIES,tagSet.IRR_TYPE_S);
		end
		...
		if ((cur>0&&cur<=len&&pcheck(str,cur-1,"l23")!=0)	&&pcheck(str,cur,"23")!=0	&&pcheck(str,cur+1,"r23")!=0) then
			...
			buf = new_str[0,cur]
			buf2 = new_str[cur..-1]
			@mc.phoneme_change(from,buf,buf2,TagSet.TAG_TYPE_YONGS,TagSet.TAG_TYPE_EOMIES,tagSet.IRR_TYPE_B);
		end
		...
		if ((cur>0&&cur+1<len&&pcheck(str,cur-1,"l26")!=0)	&&pcheck(str,cur,"26")!=0 &&pcheck(str,cur+1,"r26")!=0) then
			...
			buf = new_str[0,cur+2]
			buf2 = new_str[cur+2..-1]
			@mc.phoneme_change(from,buf,buf2,TagSet.TAG_TYPE_YONGS,TagSet.TAG_TYPE_EOMIES,tagSet.IRR_TYPE_H);
		end
    ...
    # '르' irregular rule */
		if ((cur>0&&cur<len&&pcheck(str,cur-1,"l29")!=0)&&pcheck(str,cur,"29")!=0&&pcheck(str,cur+1,"r29")!=0) then
			new_str = replace(str,cur,"ᅳ");
			if (new_str[cur+1]=='ᅡ') then
				new_str = new_str[0, cur+1] + 'ᅥ' + new_str[cur+2]
			end
      new_str = insert(new_str,cur+1,"ᄋ");
			new_str = new_str[0, cur-1] + Code.toChoseong(new_str[cur-1]) + new_str[cur..-1]

			buf = new_str[0,cur+1]
			buf2 = new_str[cur+1..-1]
			@mc.phoneme_change(from,buf,buf2,TagSet.TAG_TYPE_YONGS,TagSet.TAG_TYPE_EOMIES,tagSet.IRR_TYPE_REU);
		end
		
		# '러' irregular rule */
		if ((cur>0&&cur<=len&&pcheck(str,cur-1,"l30")!=0)&&pcheck(str,cur,"30")!=0&&pcheck(str,cur+1,"r30")!=0&&(cur-2>=0&&str[cur-2]=='ᄅ')) then
			new_str=replace(str,cur,"ᄋ");
			buf = new_str[0,cur]
			buf2 = new_str[cur..-1]
			@mc.phoneme_change(from,buf,buf2,TagSet.TAG_TYPE_YONGS,TagSet.TAG_TYPE_EOMIES,tagSet.IRR_TYPE_REO);
		end
	end
	...
	def rule_irr_word2(from,prev,str,cur)
		...
		#'여' irregular rule */
		if ((cur>0&&pcheck(str,cur-1,"l32")!=0)&&pcheck(str,cur,"32")!=0&&pcheck(str,cur+1,"r32")!=0&&str[cur+2]=='ᅧ') then
	...
	def rule_johwa(from,prev,str,cur)	
		if (cur>0&&pcheck(str,cur-1,"양성모음")!=0) then
			if (cur+2<str.length()&&str[cur+1]=='ᄋ'&&str[cur+2]=='ᅡ') then
				new_str=replace(str,cur+2,"ᅥ");
				buf = new_str[0,cur+1]
				buf2 = new_str[cur+1..-1]
				@mc.phoneme_change(from,buf,buf2,TagSet.TAG_TYPE_YONGS,TagSet.TAG_TYPE_EOMIES,0);
			elsif (cur+1<str.length()&&str[cur]=='ᄋ'&&str[cur+1]=='ᅡ') then
				new_str=replace(str,cur+1,"ᅥ");
				buf = new_str[0,cur]
				buf2 = new_str[cur..-1]
				@mc.phoneme_change(from,buf,buf2,TagSet.TAG_TYPE_YONGS,TagSet.TAG_TYPE_EOMIES,0);
	...
	def rule_NP(from,prev,str) 
		...
		if (strncmp(str,0,"내가",0,4)==0) then
			@mc.phoneme_change(from,"나",str+2,TagSet::TAG_TYPE_NBNP,TagSet::TAG_TYPE_JOSA,0);
		elsif (strncmp(str,0,"네가",0,4)==0) then
			@mc.phoneme_change(from,"너",str+2,TagSet::TAG_TYPE_NBNP,TagSet::TAG_TYPE_JOSA,0);
		elsif (strncmp(str,0,"제가",0,4)==0) then
			@mc.phoneme_change(from,"저",str+2,TagSet::TAG_TYPE_NBNP,TagSet::TAG_TYPE_JOSA,0);
		elsif (strcmp(str,0,"내",0)==0) then
			@mc.phoneme_change(from,"나","의",TagSet::TAG_TYPE_NBNP,TagSet::TAG_TYPE_JOSA,0);
		elsif (strcmp(str,0,"네",0)==0) then
			@mc.phoneme_change(from,"너","의",TagSet::TAG_TYPE_NBNP,TagSet::TAG_TYPE_JOSA,0);
		elsif (strcmp(str,0,"제",0)==0) then
			@mc.phoneme_change(from,"저","의",TagSet::TAG_TYPE_NBNP,TagSet::TAG_TYPE_JOSA,0);
		elsif (strncmp(str,0,"내게",0,4)==0) then
			@mc.phoneme_change(from,"나",buf,TagSet::TAG_TYPE_NBNP,TagSet::TAG_TYPE_JOSA,0);
		elsif (strncmp(str,0,"네게",0,4)==0) then
			buf = "에" + str[2..-1]
			@mc.phoneme_change(from,"너",buf,TagSet::TAG_TYPE_NBNP,TagSet::TAG_TYPE_JOSA,0);
		elsif (strncmp(str,0,"제게",0,4)==0) then
			buf = "에" + str[2..-1]
			@mc.phoneme_change(from,"저",buf,TagSet::TAG_TYPE_NBNP,TagSet::TAG_TYPE_JOSA,0);
		elsif (strncmp(str,0,"나",0,2)==0) then
			if (str.length()==3&&str[2]=='ᆫ') then
				@mc.phoneme_change(from,"나","는",TagSet::TAG_TYPE_NBNP,TagSet::TAG_TYPE_JOSA,0);
			elsif (str.length() == 3&&str[2]=='ᆯ') then
				@mc.phoneme_change(from,"나","를",TagSet::TAG_TYPE_NBNP,TagSet::TAG_TYPE_JOSA,0);
			end
		elsif (strncmp(str,0,"너",0,2)==0) then
			if (str.length() == 3&&str[2]=='ᆫ') then
				@mc.phoneme_change(from,"너","는",TagSet::TAG_TYPE_NBNP,TagSet::TAG_TYPE_JOSA,0);
			elsif (str.length() == 3&&str[2]=='ᆯ') then
				@mc.phoneme_change(from,"너","를",TagSet::TAG_TYPE_NBNP,TagSet::TAG_TYPE_JOSA,0);
			end
		elsif (strncmp(str,0,"누구",0,4)==0) then
			if (str.length() == 5&&str[4]=='ᆫ') then
				@mc.phoneme_change(from,"누구","는",TagSet::TAG_TYPE_NBNP,TagSet::TAG_TYPE_JOSA,0);
			elsif (str.length() == 5&&str[4]=='ᆯ') then
				@mc.phoneme_change(from,"누구","를",TagSet::TAG_TYPE_NBNP,TagSet::TAG_TYPE_JOSA,0);
			end
		elsif (strcmp(str,0,"무언가",0)==0) then
			@mc.phoneme_change(from,"무엇","인가",TagSet::TAG_TYPE_NBNP,TagSet::TAG_TYPE_JOSA,0);
		end
	end
	...
	def rule_rem(from,prev,str,cur) 
		...
		# 'ㅡ' elision rule */
		if ((cur>0&&pcheck(str,cur-1,"l12")!=0)&&pcheck(str,cur,"12")!=0&&pcheck(str,cur+1,"r12")!=0||(cur==1&&str[cur]!='ᅡ')) then
			new_str = replace(str,cur,"ᅥ");
	...
	def rule_shorten(from,prev,str,cur)
		...
		# 'ㅣ' contraction rule */
		if (cur>0) then
			if (((cur>1 || (str[cur-1]!='ᄋ'))&&pcheck(str,cur-1,"l53")!=0)&&pcheck(str,cur,"53")!=0&&pcheck(str,cur+1,"r53")!=0) then
	...
	def  strcmp(s1, i1, s2, i2)
      ...
      len-=1
		...
		return s1[i1-1].ord-s2[i2-1].ord
	end


	def strncmp(s1, i1, s2, i2, len)
